...
5. `docs/ux/` — All Phase 5 UX design artifacts if present (read every file present — wireframes and user journeys define the full scope of frontend tasks)
   `docs/visuals/ux/` — All Phase 5 HTML wireframe files if present (read every `SCR-XXX-*.html` file — these are the authoritative visual specifications for frontend tasks; Use them to accurately size frontend tasks.)
...
> **Wireframe Reference field — Frontend tasks only:** Every task of Type `Frontend` that implements or touches a screen must reference the corresponding `docs/visuals/ux/SCR-XXX-*.html` file. This field is the direct link between the task backlog and the Phase 5 visual specification. The implementation agent reads this file to understand component structure, Tailwind class patterns, DOM hierarchy, and all required states before writing any code. If the wireframe file does not exist for a screen, flag this as a Tier 1 gap before committing to the task estimate.
...
TASK-XXX
  ...
  Wireframe Reference: [docs/visuals/ux/SCR-XXX-screen-name.html — for frontend tasks only; omit for backend/DB/DevOps tasks]
...
| ID | Title | Type | Estimate | Wireframe | Depends On | Status |
|----|-------|------|----------|-----------|------------|--------|
| TASK-001 | [title] | Backend | M | — | — | ⬜ |
| TASK-002 | [title] | Frontend | S | SCR-001 | TASK-001 | ⬜ |
