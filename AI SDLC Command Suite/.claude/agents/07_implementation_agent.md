...
- **For `Frontend` type tasks:** Read the wireframe file specified in the task's `Wireframe Reference` field (`docs/visuals/ux/SCR-XXX-*.html`). Extract from it: the DOM structure, Tailwind class patterns, component hierarchy, interactive element labels, and every state variant (default, loading, empty, error, success). The Tailwind classes in the HTML wireframe are the intended implementation — replicate them in your React/Next.js components. If the wireframe reference is missing from the task, locate the matching SCR file by screen name before proceeding.
...
- Read `docs/visuals/ux/SCR-XXX-[screen-slug].html` (from the task's `Wireframe Reference` field) as the primary implementation spec before writing any component code
- **Replicate the Tailwind class patterns** from the wireframe HTML directly into your React/Next.js components — do not invent new class combinations when the wireframe already specifies them.
- **Implement every state variant** shown in the wireframe file: default, loading (skeleton), empty, error, and success where present. A component missing any state that exists in the wireframe is an incomplete implementation.
- **Preserve the DOM hierarchy** from the wireframe — the nesting order and element types carry semantic and accessibility meaning. Do not flatten or restructure the component tree without an explicit reason.
- **Annotations in the wireframe** (HTML comments `<!-- @component -->`, `<!-- @data-source -->`, `<!-- @interaction -->`) are implementation instructions — read and act on them.
